...
- **Sharing:** `packages/core` exports Drizzle schemas (via `drizzle-orm/sqlite-core`, driver-agnostic) + Zod + domain logic + a single `createDb()` (better-sqlite3 + WAL + busy_timeout). `apps/web` and `packages/mcp` depend on it with `"@cortex/core": "workspace:*"`. Dependencies flow one way (`* → core`), so no cycles. ([pnpm workspaces](https://pnpm.io/workspaces))
- **One Docker image:** multi-stage BuildKit build with the pnpm store cache mount, `pnpm deploy --filter=@cortex/web --prod` to assemble a slim runtime layer, SQLite file on a `VOLUME`, migrations run at container start. ([pnpm Docker](https://pnpm.io/docker))
...
- **Config** (`drizzle.config.ts`): `dialect: "sqlite"`, `schema`, `out: "./drizzle"`, and for `migrate`/`push` a `dbCredentials: { url: "./data/cortex.db" }` (the SQLite file path). ([drizzle config](https://orm.drizzle.team/docs/drizzle-config-file))
...
- WAL is still **single-writer**: it removes reader/writer contention but does not allow concurrent writers. For cortex's two-process case (web app + stdio MCP on one file), WAL + a nonzero `busy_timeout` is the standard mitigation for `SQLITE_BUSY`. ([sqlite WAL](https://www.sqlite.org/wal.html))
...
pnpm workspaces are enabled by a root `pnpm-workspace.yaml`. The `workspace:` protocol pins a dependency to a local workspace package and *refuses* to resolve to the registry, guaranteeing `apps/web` and `packages/mcp` always get the local `@cortex/core`. ([pnpm workspaces](https://pnpm.io/workspaces), [workspace protocol](https://pnpm.io/workspaces#workspace-protocol-workspace))

Dependency graph (one direction only):

```
apps/web   ──▶ @cortex/core
packages/mcp ─▶ @cortex/core
@cortex/core ─▶ (nothing from the workspace)
```
...
3. `apps/web` and `packages/mcp` declare `"@cortex/core": "workspace:*"` and import `{ createDb, tables, zodSchemas }` from it. ([workspace protocol](https://pnpm.io/workspaces#workspace-protocol-workspace))
...
RUN pnpm -r build
RUN pnpm deploy --filter=@cortex/web --prod /prod/web

FROM base AS runner
ENV DB_PATH=/data/cortex.db NODE_ENV=production
...
services:
  cortex:
    ...
    environment:
      ...
      - CORTEX_MCP_TOKEN=${CORTEX_MCP_TOKEN:?set a token}
    volumes:
      - cortex-data:/data
volumes:
  cortex-data:
...
#    then:  "dependencies": { "@cortex/core": "workspace:*" }

# 4. mcp package
mkdir -p packages/mcp/src
# packages/mcp/package.json
#   "name": "@cortex/mcp",
#   "dependencies": { "@cortex/core": "workspace:*", "@modelcontextprotocol/sdk": "..." }
...
export default defineConfig({
  ...
  out: "./drizzle",
  dbCredentials: { url: process.env.DB_PATH ?? "./data/cortex.db" },
});
TS
...
│       └── src/server/           # createServerFn() + createDb(@cortex/core)
├── packages/
│   ├── core/                     # @cortex/core: sqlite-core tables + zod + domain + createDb()
│   │   └── src/schema.ts, db.ts, domain/*.ts
│   └── mcp/                      # @cortex/mcp: cortex.* tools over @cortex/core
│       └── src/index.ts
└── README.md, LICENSE
```
